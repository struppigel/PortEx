 ...
 ******************************************************************************/
package io.github.struppigel.parser.coffheader;
...
import static io.github.struppigel.parser.coffheader.COFFHeaderKey.*;
...
public class COFFFileHeader extends Header<COFFHeaderKey> {
    ...
    private void read() {
        ...
        final int offset = 2;
        final int length = 3;
        IOUtil.SpecificationFormat format = new IOUtil.SpecificationFormat(key, description,
                offset, length);
    ...
    public String getInfo() {
        ...
        StringBuilder b = new StringBuilder("----------------" + IOUtil.NL
                + "COFF File Header" + IOUtil.NL + "----------------" + IOUtil.NL);
    ...
    public List<FileCharacteristic> getCharacteristics() {
        long value = get(COFFHeaderKey.CHARACTERISTICS);
    ...
    public boolean hasCharacteristic(FileCharacteristic characteristic) {
        return (get(COFFHeaderKey.CHARACTERISTICS) & characteristic.getValue()) != 0;
    }
    ...
    public MachineType getMachineType() {
        long value = get(COFFHeaderKey.MACHINE);
        try {
    ...
    public int getSizeOfOptionalHeader() {
        assert get(COFFHeaderKey.SIZE_OF_OPT_HEADER) == (int) get(COFFHeaderKey.SIZE_OF_OPT_HEADER);
    ...
    public int getNumberOfSections() {
        assert get(COFFHeaderKey.SECTION_NR) == (int) get(COFFHeaderKey.SECTION_NR);
        // 2-byte value can be casted to int
