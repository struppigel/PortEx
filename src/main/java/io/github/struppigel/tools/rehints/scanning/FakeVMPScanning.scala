...
import io.github.struppigel.tools.rehints.ReHintScannerUtils.{constructReHintIfAnySectionName, optionToList}
...
trait FakeVMPScanning extends ReHintScanner {
  ...
  private def checkSectionNames(): List[ReHint] = {
    val sectionNames = List(".vmp0")
    optionToList(constructReHintIfAnySectionName(sectionNames, data, ReHintType.FAKE_VMP_RE_HINT))
  }
