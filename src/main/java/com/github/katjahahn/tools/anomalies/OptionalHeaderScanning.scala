...
trait OptionalHeaderScanning extends AnomalyScanner {
  ...
  private def checkEntryPoint(opt: OptionalHeader): List[Anomaly] = {
    ...
    if (ep == 0 && !isDLL) {
      ...
      anomalyList += WrongValueAnomaly(entry, description, ZERO_EP)
    }
    if (ep != 0 && ep < sizeOfHeaders) {
      ...
      anomalyList += WrongValueAnomaly(entry, description, TOO_SMALL_EP)
    }
    if (isVirtual(ep)) { //TODO add more virtual entry point anomalies
      ...
      anomalyList += WrongValueAnomaly(entry, description, VIRTUAL_EP)
    }
    anomalyList.toList
  }
  ...
  private def checkLowAlignment(opt: OptionalHeader): List[Anomaly] = {
    ...
    if (isLowAlignment(sectionAlignment, fileAlignment)) {
      ...
      anomalyList += NonDefaultAnomaly(entry, description, LOW_ALIGNMENT_MODE)
    }
    anomalyList.toList
  }
  ...
  private def checkSizes(opt: OptionalHeader): List[Anomaly] = {
    ...
    if (sectionAlignment != 0 && imageSize % sectionAlignment != 0) {
      ...
      anomalyList += WrongValueAnomaly(entry, description, NOT_SEC_ALIGNED_SIZE_OF_IMAGE)
    }
    ...
    if (fileAlignment != 0 && headerSize % fileAlignment != 0) {
      ...
      anomalyList += WrongValueAnomaly(headerSizeEntry, description, NOT_FILEALIGNED_SIZE_OF_HEADERS)
    ...
    if (headerSize < headerSizeMin) {
      ...
      anomalyList += WrongValueAnomaly(headerSizeEntry, description, TOO_SMALL_SIZE_OF_HEADERS)
    }
    if (headerSize != roundedUpHeaderSize) {
      ...
      anomalyList += NonDefaultAnomaly(headerSizeEntry, description, NON_DEFAULT_SIZE_OF_HEADERS)
    }
    anomalyList.toList
  }
  ...
  private def checkReservedAndDeprecated(opt: OptionalHeader): List[Anomaly] = {
    ...
    for (ch <- dllChs if ch.isReserved || ch.isDeprecated) {
      ...
      if (ch.isReserved) {
        ...
        List(ReservedAnomaly(entry, description, RESERVED_DLL_CHARACTERISTICS))
      } else {
        val description = s"Optional Header: Deprecated DllCharacteristic ${ch.toString()} is not 0"
        List(DeprecatedAnomaly(entry, description, RESERVED_DLL_CHARACTERISTICS))
      }
    }
    if (win32version != 0) {
      ...
      List(ReservedAnomaly(entry, description, RESERVED_WIN32VERSION))
    }
    if (loaderFlags != 0) {
      ...
      List(ReservedAnomaly(entry, description, RESERVED_LOADER_FLAGS))
    }
    anomalyList.toList
  }
  ...
  private def checkFileAlignment(opt: OptionalHeader): List[Anomaly] = {
    ...
    if (!isPowerOfTwo(fileAlignment)) {
      ...
      anomalyList += WrongValueAnomaly(entry, description, NOT_POW_OF_TWO_FILEALIGN)
    }
    ...
    if (fileAlignment != 512) {
      ...
      anomalyList += NonDefaultAnomaly(entry, description, NON_DEFAULT_FILEALIGN)
    }
    anomalyList.toList
  }
  ...
  private def checkSectionAlignment(opt: OptionalHeader): List[Anomaly] = {
    ...
    if (sectionAlignment < fileAlignment) {
      ...
      List(WrongValueAnomaly(entry, description, TOO_SMALL_SECALIGN))
    } else Nil
  }
  ...
  private def checkImageBase(opt: OptionalHeader): List[Anomaly] = {
    ...
    if (imageBase % 65536 != 0) {
      ...
      anomalyList += WrongValueAnomaly(entry, description, NOT_MULT_OF_64K_IMAGE_BASE)
    }
    if ((imageBase + sizeOfImage) >= 0x80000000L) {
      ...
      anomalyList += NonDefaultAnomaly(entry, description, TOO_LARGE_IMAGE_BASE)
    }
    if (imageBase == 0) {
      val description = s"Optional Header: The image base is 0, thus relocated to 0x10000"
      anomalyList += NonDefaultAnomaly(entry, description, ZERO_IMAGE_BASE)
    }
    if (isDLL() && imageBase != 0x10000000L) {
      ...
      anomalyList += NonDefaultAnomaly(entry, description, NON_DEFAULT_IMAGE_BASE)
  ...
  private def dataDirScan(opt: OptionalHeader): List[Anomaly] = {
    ...
    if (datadirs.size() != 16) {
      ...
      if (entry.value != 16) {
        ...
        anomalyList += NonDefaultAnomaly(entry, description, UNUSUAL_DATA_DIR_NR)
      }
    }
